      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        <div className="bg-white p-8 rounded-3xl shadow-lg border border-slate-100 max-w-md w-full">
          ...
          <p className="text-sm font-medium text-slate-500 leading-relaxed mb-6">
            You are logged in as a <strong className="text-slate-700 capitalize">{dbUser.role.replace('_', ' ')}</strong>, but this portal requires <strong className="text-slate-700 capitalize">{allowedRoles[0].replace('_', ' ')}</strong> privileges.
          </p>
          <div className="flex flex-col gap-3">
             <button
              onClick={() => {
                if (dbUser.role === 'student') window.location.href = 'https://uni-uganda5.vercel.app/';
                else if (dbUser.role === 'admin') window.location.href = 'https://uni-uganda6.vercel.app/';
                else window.location.href = 'https://uni-uganda4.vercel.app/';
              }}
