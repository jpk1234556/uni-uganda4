...
export default function Navbar({ appType }: { appType?: "student" | "owner" | "admin" }) {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full transition-all duration-300 shadow-sm bg-white/95 backdrop-blur-md supports-[backdrop-filter]:bg-white/60 border-b border-slate-200">
      <div className="container mx-auto flex h-20 items-center justify-between px-4 lg:px-8">
        ...
        <div className="hidden lg:flex items-center space-x-8 text-sm font-semibold text-slate-600">
          ...
          {isOwnerApp && (
            <Link to="/owner/dashboard" className="text-primary hover:text-primary/80 transition-colors focus:outline-none flex items-center gap-2">
              Management Portal
            </Link>
          )}
        </div>
        ...
        <div className="hidden lg:flex items-center space-x-4 ml-auto">
          ...
          {user ? (
            ...
            <Link to="/auth">
              <Button variant="outline" className="rounded-full shadow-sm gap-2 transition-all border-slate-200 bg-white hover:bg-slate-50 hover:text-primary hover:border-primary/20 text-slate-700">
      ...
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 top-20 z-40 border-t flex flex-col p-4 animate-in slide-in-from-top-4 duration-200 bg-white border-slate-100">
          {!isOwnerApp && (
          ...
          <div className="flex flex-col space-y-4 mb-8">
            ...
            {isOwnerApp && (
              <Link to="/owner/dashboard" onClick={() => setIsMobileMenuOpen(false)} className="text-lg font-bold text-primary p-2 rounded-lg bg-primary/5 transition-colors">
                Management Portal
              </Link>
            )}
          </div>

          <div className="mt-auto border-t pt-6 flex flex-col gap-3 border-slate-100">
             {user ? (
               <>
                 <div className="text-center font-medium mb-2 text-slate-500">
               ...
               <Link to="/auth" onClick={() => setIsMobileMenuOpen(false)}>
                 <Button className="w-full h-12 text-lg rounded-xl transition-opacity bg-gradient-primary hover:opacity-90">
                   Sign In / Register
                 </Button>
               </Link>
             )}
