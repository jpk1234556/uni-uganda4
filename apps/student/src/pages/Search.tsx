...
import { SearchIcon, MapPin, Wifi, Shield, Zap, Car, Star, Heart } from "lucide-react";
...
export default function Search() {
  ...
  const sortedHostels = [...filteredHostels].sort((a, b) => {
    if (sortBy === "price-asc") {
      const minA = a.room_types && a.room_types.length > 0 ? Math.min(...a.room_types.map(r => r.price)) : Infinity;
      const minB = b.room_types && b.room_types.length > 0 ? Math.min(...b.room_types.map(r => r.price)) : Infinity;
      return minA - minB;
    }
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-1/4 space-y-6">
          <div className="bg-white/5 backdrop-blur-md p-6 rounded-2xl shadow-sm border border-white/10">
            ...
            <Button 
              className="w-full mt-8 bg-gradient-primary text-white hover:opacity-90 shadow-md"
              onClick={() => {
                setSearchTerm("");
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Link to={`/hostel/${hostel.id}`} key={hostel.id}>
                    ...
                    <div className="aspect-[4/3] relative overflow-hidden bg-muted">
                        ...
                        >
                          <Heart className={cn("h-5 w-5", favorites.includes(hostel.id) && "fill-current")} />
                        </Button>
                    </div>
