...
export default function HostelVerification() {
  ...
  const handleUpdateStatus = async (hostelId: string, status: "approved" | "rejected") => {
    try {
      ...
      toast.success(`Hostel ${status === "approved" ? "approved" : "rejected"} successfully`);
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">Security_Protocol</span>
          </div>
          <h2 className="text-2xl font-black tracking-tighter text-slate-900 uppercase">Hostel_Verification</h2>
          <p className="text-slate-500 text-[10px] font-mono mt-1 uppercase tracking-widest">REVIEW_AND_VALIDATE_NEW_LISTING_REQUESTS</p>
        </div>
      </div>
      ...
      <Card className="border-slate-200 rounded-none shadow-sm bg-white overflow-hidden">
        <div className="bg-slate-50/50 border-b border-slate-200 p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xs font-bold text-slate-900 uppercase tracking-widest">Pending_Approvals</h3>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            <div className="py-20 flex flex-col items-center justify-center gap-3">
              ...
              <span className="text-[10px] font-mono text-slate-400 uppercase tracking-widest">Accessing_Data_Streams...</span>
            </div>
          ) : pendingHostels.length === 0 ? (
            <div className="py-20 text-center flex flex-col items-center justify-center">
              ...
              <h3 className="text-xs font-bold text-slate-900 uppercase tracking-widest">System_Clear</h3>
              <p className="text-[10px] text-slate-400 uppercase tracking-wider font-mono mt-1">NO_PENDING_VERIFICATIONS_IN_QUEUE.</p>
            </div>
          ) : (
            <Table>
              <TableHeader className="bg-slate-50/50 border-b border-slate-200">
                <TableRow className="hover:bg-transparent border-none">
                  <TableHead className="h-10 text-[10px] font-bold uppercase tracking-widest text-slate-500">Hostel_Details</TableHead>
                  <TableHead className="h-10 text-[10px] font-bold uppercase tracking-widest text-slate-500">Owner_Information</TableHead>
                  <TableHead className="h-10 text-[10px] font-bold uppercase tracking-widest text-slate-500">Location_Data</TableHead>
                  <TableHead className="h-10 text-[10px] font-bold uppercase tracking-widest text-slate-500 text-right">Decision_Matrix</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {pendingHostels.map((hostel) => (
                  <TableRow key={hostel.id} className="hover:bg-slate-50/30 border-b border-slate-100 transition-colors group">
                    ...
                    <TableCell className="py-4">
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="text-[10px] text-slate-400 font-mono">{hostel.users?.email}</span>
                      </div>
                    </TableCell>
                    <TableCell className="py-4">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-1.5 text-slate-600">
                          ...
                          <span className="text-[10px] font-bold uppercase tracking-widest">{hostel.university}</span>
                        </div>
                        <span className="text-[10px] text-slate-400 font-mono uppercase tracking-wider">{hostel.address}</span>
                      </div>
                    </TableCell>
        ...
        <DialogContent className="sm:max-w-[600px] rounded-none border-slate-200 font-mono max-h-[90vh] overflow-y-auto">
          <DialogHeader className="border-b border-slate-100 pb-4">
            <DialogTitle className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest">
              <ShieldCheck className="h-4 w-4 text-indigo-500"/> Property_Inspection_Protocol
            </DialogTitle>
          ...
          <div className="py-4 space-y-6">
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-slate-200">
                {selectedHostel?.images && selectedHostel.images.length > 0 ? (
                  selectedHostel.images.map((img: string, idx: number) => (
                    <div key={idx} className="h-32 w-48 shrink-0 bg-slate-100 border border-slate-200 overflow-hidden">
                      <img 
                        src={img} 
                        alt={`Asset_${idx}`} 
            ...
            <div className="space-y-3">
              ...
              <div className="p-4 bg-slate-50 border border-slate-100">
                <h4 className="text-xs font-bold text-slate-900 uppercase mb-2">{selectedHostel?.name}</h4>
                <p className="text-[10px] text-slate-600 leading-relaxed uppercase">
                  {selectedHostel?.description || "NO_DESCRIPTION_DATA_AVAILABLE"}
                </p>
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {selectedHostel?.amenities && selectedHostel.amenities.length > 0 ? (
                  selectedHostel.amenities.map((amenity: string, idx: number) => (
                    <span key={idx} className="px-2 py-1 bg-white border border-slate-200 text-[9px] font-bold text-slate-600 uppercase tracking-widest">
                      {amenity}
                    </span>
                  ))
                ) : (
                  <span className="text-[10px] text-slate-400 uppercase tracking-widest italic">NO_AMENITIES_LISTED</span>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Geographic_Data</span>
                <div className="text-[10px] text-slate-700 font-bold uppercase">{selectedHostel?.university}</div>
                <div className="text-[10px] text-slate-500 uppercase">{selectedHostel?.address}</div>
              </div>
              <div className="space-y-2">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Owner_Identity</span>
                <div className="text-[10px] text-slate-700 font-bold uppercase">
                  {selectedHostel?.users?.first_name} {selectedHostel?.users?.last_name}
                </div>
                <div className="text-[10px] text-slate-500 lowercase">{selectedHostel?.users?.email}</div>
              </div>
            </div>
          </div>
          ...
          <DialogFooter className="border-t border-slate-100 pt-4 flex gap-2">
            ...
            >
              {processingId === selectedHostel?.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Reject_Listing"}
            </Button>
            <Button 
            ...
            >
              {processingId === selectedHostel?.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Approve_Listing"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
