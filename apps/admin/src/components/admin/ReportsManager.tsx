...
export default function ReportsManager() {
  ...
  const [dateRange, setDateRange] = useState<
    "this_month" | "last_30_days" | "all_time"
  >("this_month");
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-2 text-lg">
            Generate and export platform performance metrics and revenue
            reports.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Button
            ...
            variant="outline"
            className="gap-2 bg-white text-slate-600 border-slate-200 hover:bg-slate-50 shadow-sm"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
        <motion.div variants={itemVariants}>
          <Card className="border-slate-200 shadow-sm bg-white overflow-hidden rounded-xl h-80 flex flex-col items-center justify-center p-8 outline-dashed outline-2 outline-slate-200 outline-offset-[-16px]">
            ...
            <p className="text-slate-500 text-center mt-2 max-w-sm">
              Detailed financial plotting will be available once the mobile
              money integration captures 30 days of data.
            </p>
          </Card>
        </motion.div>
        ...
        <motion.div variants={itemVariants}>
          <Card className="border-slate-200 shadow-sm bg-white overflow-hidden rounded-xl h-80 flex flex-col items-center justify-center p-8 outline-dashed outline-2 outline-slate-200 outline-offset-[-16px]">
            ...
            <p className="text-slate-500 text-center mt-2 max-w-sm">
              Visual breakdown of Students vs Hostel Owners currently active on
              the platform.
            </p>
          </Card>
        </motion.div>
      ...
      <motion.div variants={itemVariants} className="mt-8">
        <Card className="border-indigo-100 shadow-md bg-gradient-to-r from-indigo-50 to-blue-50 overflow-hidden rounded-xl">
          <CardContent className="p-8">
            ...
            <p className="text-indigo-700/80 mb-6 max-w-2xl">
              Configure the system to automatically email detailed PDF reports
              on the 1st of every month summarizing total bookings, completed
              payments, and new user registrations.
            </p>
            <Button
              onClick={handleConfigureAutomation}
              className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm font-medium px-6"
            >
